...
def Riskpull(url):
    ...
    regexs = ('bold;\">\s*Item 1A\.(.+?)bold;\">\s*Item 1B\.',
    'bold;\">\s*Item 1A\:(.+?)bold;\">\s*Item 1B\:',
    'bold;\">\s*Item 1A\-(.+?)bold;\">\s*Item 1B\-',
    'bold;\">\s*Item IA\.(.+?)bold;\">\s*Item IB\.',
    'bold;\">\s*Item 1A.(.+?)bold;\">\s*Item 1B.')
    ...
    for regex in regexs:
        ...
        if match:
            ...
            outfile = open(path1 + 'Risk/' + cik + '.txt','a')
...
path1 = '/Users/mhisten/Documents/python/SEC2/'
os.chdir(path1)
